...
const checks = [
  ...
  ['따뜻한 화면 분위기 3종 제공', page.includes("'sunset'") && page.includes("'cream'") && page.includes("'wood'")],
  ['기존 장밋빛 저장값 우드톤 전환', page.includes("savedThemeTone === 'rose'") && page.includes("localStorage.setItem(THEME_TONE_STORAGE_KEY, 'wood')")],
  ['화면 색상 선택값 실제 반영', page.includes('data-theme-tone={themeTone}') && page.includes('THEME_TONE_STORAGE_KEY') && !page.includes('선택값은 아직 화면 색감에 반영하지 않습니다')],
  ...
  ['마이페이지 설정 스타일 유지', styles.includes('.toggle-button') && styles.includes('.theme-options')],
  ['화면 색상 테마 스타일', styles.includes(".page-shell[data-theme-tone='cream']") && styles.includes(".page-shell[data-theme-tone='wood']") && styles.includes('--theme-background-layer')],
