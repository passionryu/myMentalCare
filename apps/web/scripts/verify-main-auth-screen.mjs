...
const checks = [
  ...
  ['통계 진입 버튼 디자인 클래스 유지', page.includes('statistics-entry-card') && page.includes('statistics-entry-arrow') && page.includes("router.push('/statistics')")],
  ['체크인 섹션 Simple Start 제목 적용', page.includes('Simple Start') && page.includes('simple-start-title') && !page.includes('Check In으로 시작하기')],
  ...
  ['Simple Start 브랜드형 스타일', styles.includes('.simple-start-title') && styles.includes('.simple-start-title::after')],
  ['통계 버튼 hover 스타일', styles.includes('.statistics-entry-arrow') && styles.includes('.statistics-entry-card:hover .statistics-entry-arrow')],
