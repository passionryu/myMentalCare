...
internal class AiChatMessageAppender(
    ...
    fun appendUserMessage(room: AiChatRoom, content: String, crisisDetected: Boolean): ChatMessage {
        return appendMessage(room, ChatMessageSenderType.USER, content.trim(), crisisDetected)
    }

    // 챗봇 응답 메시지를 현재 대화방의 다음 순서로 저장한다.
    fun appendAssistantMessage(room: AiChatRoom, content: String, crisisDetected: Boolean): ChatMessage {
        return appendMessage(room, ChatMessageSenderType.ASSISTANT, content, crisisDetected)
    }
